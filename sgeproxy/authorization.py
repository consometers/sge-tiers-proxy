...
class Authorizations:
    ...
    def set(self, user_id, usage_point_id, authorization):
        if user_id not in self.authorizations:
            self.authorizations[user_id] = {}
        assert usage_point_id not in self.authorizations[user_id]
    ...
    def validate(self, user_id, usage_point_id, measurement_scope=None):
        ...
        if measurement_scope:
            ...
            exclusions = authorization.measurement_scope.get_exclusions(
                measurement_scope
            )
            if exclusions:
                raise PermissionError(
                    f"{user_id} is not allowed to access {usage_point_id} "
                    + ", ".join(exclusions)
                )
...
class Authorization:
    ...
    def from_conf(cls, conf):
        return cls(
            quoalise.parse_iso_date(conf["begins_at"]),
...
class MeasurementScope:
    def __init__(
        self, details=False, consumption=False, production=False, history=None
    ):
