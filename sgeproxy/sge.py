...
if __name__ == "__main__":
    ...
    # FIXME(cyril) move to intgration tests
    import argparse
    ...
    parser.add_argument(
        "measurement", help="Measurement name (like consumption/active_power/raw)"
    )
    ...
    data = api.get_mesurements(
        args.measurement, args.prm, args.start_date, args.end_date
    )

    print(tostring(data.xml))
    ...
    assert (
        ...
        == dt.datetime.strptime(
            "2021-02-01 00:00:00+0100", "%Y-%m-%d %H:%M:%S%z"
        ).timestamp()
    )
